...
export function QuizResults() {
  ...
  const getScoreBadge = (score: number) => {
    ...
    if (score >= 80) return { label: 'Great Job!', variant: 'secondary' as const };
    if (score >= 70) return { label: 'Good Work!', variant: 'secondary' as const };
    if (score >= 60) return { label: 'Keep Practicing', variant: 'outline' as const };
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
              <div className="flex flex-col items-center">
                ...
                <div className="text-2xl font-bold">{correctAnswers}/{results.totalQuestions}</div>
                <div className="text-sm text-muted-foreground">Correct Answers</div>
              </div>
              
              <div className="flex flex-col items-center">
                <Clock className="w-8 h-8 text-primary mb-2" />
                <div className="text-2xl font-bold">{Math.floor(results.timeSpent / 60)}:{(results.timeSpent % 60).toString().padStart(2, '0')}</div>
                <div className="text-sm text-muted-foreground">Time Spent</div>
              </div>
              
              <div className="flex flex-col items-center">
                <Trophy className="w-8 h-8 text-primary mb-2" />
                <div className="text-2xl font-bold">{Math.round(results.timeSpent / results.totalQuestions)}s</div>
